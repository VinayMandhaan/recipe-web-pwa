...
export default function RecipeCard({ data }: { data: ExtractResponse }) {
  ...
  if (isFallback || !recipe) {
    return (
      <div className="bg-white shadow-sm rounded-2xl border border-gray-200 p-5 space-y-3">
        ...
        {data.caption && (
          <div>
            ...
            <p className="text-sm text-gray-500 whitespace-pre-wrap">{data.caption}</p>
          </div>
        )}
        <a
          href={data.url}
      ...
      <div className="bg-white shadow-sm rounded-2xl border border-gray-200 overflow-hidden">
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-2">
                ...
                {data.platform && (
                  <span className="text-[10px] text-gray-500 capitalize">{data.platform}</span>
                )}
              </div>
            </div>
            ...
            <button
              ...
              className={`p-2 rounded-xl transition-colors ${
                saved
                  ...
                  : "bg-gray-50 text-gray-500 active:bg-blue-50 active:text-blue-500"
              }`}
            >
              {saving ? (
                <span className="w-5 h-5 border-2 border-blue-500/30 border-t-blue-500 rounded-full animate-spin block" />
              ) : saved ? (
            ...
            <ol className="space-y-3">
              {recipe.steps.map((step, i) => (
                <li key={i} className="flex gap-3 text-sm text-gray-700">
                  <span className="text-blue-500 font-bold shrink-0 w-5 text-right">
                    {i + 1}
                  </span>
        ...
        <div className="px-5 py-4 border-t border-gray-200 flex gap-3">
          <button
            ...
            className="flex-1 py-2.5 bg-blue-50 text-blue-500 font-medium rounded-xl text-sm
          ...
          <button
            onClick={handleShare}
            className="flex-1 py-2.5 bg-gray-50 text-gray-500 font-medium rounded-xl text-sm
